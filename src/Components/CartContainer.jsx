...
const CartContainer = ({ handleRemoveOne, handleAddToCart, handleDeleteItem, handleDeleteCart, formOpen }) => {
  ...
  return (
    ...
    >
      <h1 className="h-16 w-full z-50 flex flex-col justify-center absolute top-0 left-0 text-left md:text-center font-bold text-lg md:text-xl uppercase pl-4 md:pl-8">
        Your Cart
      </h1>
      ...
      {items.length === 0 ? (
        ...
        <div
          style={{ height: width < 768 ? "calc(100vh - 250px)" : "calc(100vh - 140px)" }}
          className="w-11/12 absolute bottom-0 mx-auto overflow-y-auto flex flex-col items-center justify-start gap-4 pb-16 md:pb-28"
        >
          {items.map((item, i) => (
            <div
              key={i}
              className="item h-24 md:h-48 w-full flex items-center justify-start gap-1 md:gap-6 pb-4 pr-1 md:px-0 border-b border-gray-600 last:border-b-0"
            >
              <div
              ...
              <div className="details h-full w-full md:w-1/3 flex items-center justify-center gap-2">
                <div className="h-full w-full flex flex-col items-start justify-center gap-1 md:gap-2 pl-1 xl:pl-4">
                  <div className="capitalize md:uppercase md:whitespace-nowrap">{item.product.name}</div>
